...
env_paths = [
    Path(__file__).resolve().parent.parent / ".env",  # backend_chat/.env
...
for env_path in env_paths:
    if env_path.exists():
        load_dotenv(dotenv_path=env_path, override=True)  # CRITICAL: override=True to prioritize .env
        print(f"✓ Loaded .env from: {env_path}")
        break
else:
    # Final fallback: try loading from current directory without explicit path
    load_dotenv(override=True)

# Final source of truth for keys
...
# Fast local intent classification keywords
COLLEGE_KEYWORDS = {
    'syllabus', 'exam', 'examination', 'test', 'mark', 'score', 'grade', 'result',
    'attendance', 'present', 'absent', 'leave', 'od', 'duty',
    'fee', 'fees', 'fines', 'payment', 'due', 'scholarship',
    'hostel', 'mess', 'food', 'room', 'warden',
    'bus', 'transport', 'route', 'driver', 'stop',
    'library', 'book', 'journal', 'return', 'fine',
    'lab', 'laboratory', 'computer', 'equipment',
    'class', 'classroom', 'lecture', 'period', 'timetable', 'schedule',
    'professor', 'prof', 'teacher', 'faculty', 'staff', 'hod', 'principal',
    'student', 'roll', 'number', 'id', 'card',
    'event', 'function', 'sports', 'cultural', 'symposium', 'workshop',
    'placement', 'job', 'company', 'recruit', 'interview', 'salary', 'package',
    'college', 'campus', 'department', 'cse', 'ece', 'eee', 'mech', 'civil', 'aids',
    'admission', 'application', 'cutoff', 'counselling',
    'rule', 'regulation', 'discipline', 'dress', 'uniform', 'wear',
    'wifi', 'internet', 'login', 'portal', 'moodle', 'erp'
}

def classify_intent(user_text):
    """
    Classify user question as COLLEGE_SPECIFIC or GENERAL using fast local keyword matching.
    Replaces the slow Gemini API call.
    ...
    # 1. Check for specific college keywords
    words = set(re.findall(r'\w+', text_lower))
    common_words = words.intersection(COLLEGE_KEYWORDS)
    
    if common_words:
        return {"intent_type": "COLLEGE_SPECIFIC", "confidence": "HIGH"}
        
    # 2. Check for question patterns that imply college context
    # matching "term?" or "who is..." often relates to local context in this app
    if re.search(r'\b(who|where|when|what)\s+(is|are|will)\s+(the|my)\b', text_lower):
        # Slightly risky but safer to assume college context for "where is the lab" 
        # than general context which would fail to answer it.
        # But we must be careful not to capture "what is the weather"
        pass
        
    # 3. Default to GENERAL if no keywords match
    # However, if we are unsure, returning COLLEGE_SPECIFIC is often safer 
    # because it triggers the context lookup, which might return empty but won't hurt.
    # But for performance (saving DB lookups), let's stick to GENERAL if we truly found nothing.
